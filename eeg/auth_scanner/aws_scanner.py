    ...
    def _check_guardrails(self, bedrock, collector: Collector):
        ...
        for gr in guardrails:
            ...
            try:
                ...
                print(f"  [AUTH-AWS] Error auditing guardrail {gr_name}: {str(e)[:100]} - continuing to next guardrail")
                collector.add_permission_issue(f"audit_guardrail_{gr_name}", gr_name, str(e)[:150])
                continue
    ...
    def _check_agents(self, bedrock_agent, bedrock, collector: Collector):
        ...
        for ag in agents:
            ...
            try:
                ...
                print(f"  [AUTH-AWS] Error auditing agent {agent_name}: {str(e)[:100]} - continuing to next agent")
                collector.add_permission_issue(f"audit_agent_{agent_name}", agent_name, str(e)[:150])
                continue
    ...
    def _check_knowledge_bases(self, bedrock_agent, s3, collector: Collector):
        ...
        for kb in kbs:
            ...
            try:
                ...
                print(f"  [AUTH-AWS] Error auditing knowledge base {kb_name}: {str(e)[:100]} - continuing to next KB")
                collector.add_permission_issue(f"audit_kb_{kb_name}", kb_name, str(e)[:150])
                continue
